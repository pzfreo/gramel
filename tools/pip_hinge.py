...
build123d port + four-input API: Paul Fremantle (pzfreo).
All CC BY 4.0.
...
Parametric print-in-place piano hinge for clamshell cases. Four inputs,
everything else derived:
  ...
  * ``knuckle``       Knuckle.FULL  → Po = 2 × case_h, knuckle rests on bed,
                      ...
                      Knuckle.SMALL → Po = max(case_h/2, 5 mm), steep ramp
...
class HingeParams:
    ...
    Default 0.2 mm matches the upstream pegturner/clamshell example.
    Set to 0 to disable (knuckle bottom rests directly on the wall
    top, no seam gap).
    """
...
def make_hinge(params: HingeParams = None) -> Compound:
    ...
    # Effective leaf height — grows by pivot_z_offset in Z so the
    # bed-side end of the leaf still reaches the bed when the axis is
    # lifted by pivot_z_offset.
    ...
    # (W, -leaf_h) to (0, -T) lies flat on the bed; at HALF it slopes up
