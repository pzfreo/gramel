...
class ThumbwheelDriveScrewFeatures:
    """The named turned cylinders behind :func:`build_thumbwheel_drive_screw`, kept
    separate (pre-union) so drawing code can reference each one directly instead of
    restating its size/position as numbers."""
...
def build_thumbwheel_drive_screw_features(params: PurflingCutterParams) -> ThumbwheelDriveScrewFeatures:
    """Build the integral thumbwheel + drive screw, keeping each turned feature."""
